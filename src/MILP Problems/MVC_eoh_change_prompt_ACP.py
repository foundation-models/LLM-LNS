...
class PROBLEMCONST:
        ...
        try:
            while(time.time() - begin_time <= self.set_time):
                ...
                if parts > 0:
                    ...
                    # If parts is 0, select all nodes
                    for i in range(n):
                        color[indices[i]] = 1
                ...
                if(len(turn_ans) > 3 and abs(turn_ans[-1] - turn_ans[-3]) <= self.epsilon *  turn_ans[-1] and parts > 1):
                    parts -= 1
            return(turn_ans[-1])
    ...
    def greedy(self, eva):
        ...
        if len(results) > 0:
            ...
            return 1e9  # Return a large penalty value if no results
...
class Methods:
    ...
    def get_method(self):
        ...
        if self.paras.method == "eoh":   
            return EOH(self.paras,self.problem,self.select,self.manage, clear_checkpoint=clear_checkpoint)
        else:
...
# print(f"DEBUG: Using API key: {api_key[:10]}..." if api_key != "your_api_key" else "DEBUG: Using default API key")
